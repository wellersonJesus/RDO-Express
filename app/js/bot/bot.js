...
window.editarBot = (id) => {
    const item = window.botState.cache.find(b => b.id == id);
    window.botState.idEmEdicao = id;
    window.botState.origemEmEdicao = item.origem;
    document.getElementById('edit-user').value = item.username || item.responsavel || '';
    new bootstrap.Modal(document.getElementById('modalEdicao')).show();
};

window.confirmarEdicao = async () => {
    await window.API.call('update' + window.botState.origemEmEdicao, {id: window.botState.idEmEdicao, username: document.getElementById('edit-user').value});
...
window.confirmarExclusao = async () => {
    await window.API.call('delete' + window.botState.origemEmEdicao, {id: window.botState.idEmEdicao});
