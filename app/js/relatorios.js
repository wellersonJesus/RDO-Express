...
(function () {
    ...
    function definirPeriodoPadrao() {
        var hoje = new Date();
        var di = toISO(new Date(hoje.getFullYear(), hoje.getMonth(), 1));
        var df = toISO(new Date(hoje.getFullYear(), hoje.getMonth() + 1, 0));
        var ids = [
            'rel-mb-data-inicio', 'rel-mb-data-fim',
            'rel-cli-data-inicio', 'rel-cli-data-fim',
            'rel-fin-data-inicio', 'rel-fin-data-fim',
            'rel-glob-data-inicio', 'rel-glob-data-fim'
        ];
        ids.forEach(function (id) {
            var el = document.getElementById(id);
            if (el) el.value = id.indexOf('inicio') !== -1 ? di : df;
        });
    }
    ...
    function abrirModal(titulo, htmlBody, textoPlano, periodoLabel) {
        ...
        if (btnEmail) {
            btnEmail.onclick = function () {
                window.location.href = 'mailto:?subject=' + encodeURIComponent(titulo + ' — ' + periodoLabel) + '&body=' + encodeURIComponent(textoPlano);
            };
        }
    }
    ...
    function gerarRelatorioCliente() {
        ...
        if (cliSel !== '__todos__') {
            ...
            var nomeCliente = cliObj ? (cliObj.username || cliObj.nome || '') : cliSel;
    ...
    function initRelatorioTabs() {
        var tabs     = document.querySelectorAll('.rel-tab');
        var contents = document.querySelectorAll('.rel-tab-content');
        if (!tabs.length) return;
        tabs.forEach(function (tab) {
            tab.addEventListener('click', function (e) {
                e.preventDefault();
                var target = this.getAttribute('data-rel-tab');
                tabs.forEach(function (t) { t.classList.remove('active'); });
                contents.forEach(function (c) { c.classList.remove('active'); });
                this.classList.add('active');
                var content = document.getElementById('rel-tab-content-' + target);
    ...
    function carregarDados() {
        ...
        Promise.all([
            ...
            extrairArray(results[1]).forEach(function (p) {
                ...
                if (id) state._pedidosCache[id] = {
                    id: id,
                    id_cliente: (p.id_cliente || '').toString().trim(),
                    solicitante: (p.solicitante || '').toString().trim()
                };
            });
